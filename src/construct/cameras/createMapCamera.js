...
export default (layout, route, map) => {
  ...
  camera.normalizePixels = (pixels) => ({
    x:
      ...
      (pixels.x - 0.5 * canvas.width) / (canvas.height * camera.scale),
    y:
      camera.normalizedCenter.y +
      (pixels.y - 0.5 * canvas.height) / (canvas.height * camera.scale),
  });
  ...
  const transformNormalizedToPixels = (normalized) => ({
    x:
      (normalized.x - camera.normalizedCenter.x) *
        (canvas.height * camera.scale) +
      0.5 * canvas.width,
    y:
      (normalized.y - camera.normalizedCenter.y) *
        (canvas.height * camera.scale) +
      0.5 * canvas.height,
  });
  ...
  camera.moveByPixels = (pixelMovement) => {
    camera.normalizedCenter.x +=
      pixelMovement.x / (canvas.height * camera.scale);
    camera.normalizedCenter.y +=
      pixelMovement.y / (canvas.height * camera.scale);
  ...
  camera.zoomToRoute = () => {
    const boundingBox = route.segments.reduce(
      (boundingBox, segment) => {
    ...
    camera.scale = Math.min(
      canvas.width / canvas.height / sizeNormalized.x,
      1.0 / sizeNormalized.y,
    );
  ...
  camera.update = () => {
    ...
    camera.scale = Math.max(
      Math.max(canvas.width / canvas.height, 1.0),
      camera.scale,
    );
