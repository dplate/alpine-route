...
export default (layout, route, mapCamera) => {
  ...
  const leftToRight =
    route.segments[0].x < route.segments[route.segments.length - 1].x;

  camera.normalizePixels = (pixels) => ({
    flatMeter: leftToRight
      ? pixels.x / canvas.width
      : 1.0 - pixels.x / canvas.width,
    z: 1.0 - (pixels.y - profilePadding) / (canvas.height - 2 * profilePadding),
  });

  camera.normalizeMeters = (point) => ({
    flatMeter:
      (point.flatMeter - camera.minFlatMeter) /
      (camera.maxFlatMeter - camera.minFlatMeter),
    z: (point.z - camera.minHeight) / (camera.maxHeight - camera.minHeight),
  });

  camera.transformNormalizedToPixels = (normalized) => ({
    x: leftToRight
      ? normalized.flatMeter * canvas.width
      : canvas.width - normalized.flatMeter * canvas.width,
    y:
      profilePadding +
      (1.0 - normalized.z) * (canvas.height - 2 * profilePadding),
  });
  ...
  camera.transformPixelsToMeters = (pixels) => {
    ...
    return {
      flatMeter:
        normalized.flatMeter * (camera.maxFlatMeter - camera.minFlatMeter) +
        camera.minFlatMeter,
      z:
        normalized.z * (camera.maxHeight - camera.minHeight) + camera.minHeight,
    };
  };

  camera.transformPixelDistanceToHeightDifference = (pixelDistance) => {
    const normalizedZDifference =
      camera.normalizePixels({ x: 0, y: 0 }).z -
  ...
  camera.update = () => {
    ...
    const maxMapMeters = mapCamera.transformPixelsToMeters({
      x: layout.map.width,
      y: layout.map.height,
    });
    ...
    const calculateDistanceToCenter = (segment) =>
      Math.sqrt(
        (centerMapMeters.x - segment.x) ** 2 +
          (centerMapMeters.y - segment.y) ** 2,
      );
    const isVisible = (segment) =>
      segment.x >= minMapMeters.x &&
      segment.x < maxMapMeters.x &&
      segment.y >= minMapMeters.y &&
      segment.y < maxMapMeters.y;
    ...
    route.segments.forEach((segment, index) => {
      if (isVisible(segment)) {
        ...
        currentPart.minDistanceToCenter = Math.min(
          currentPart.minDistanceToCenter,
          calculateDistanceToCenter(segment),
        );
        currentPart.editing =
          currentPart.editing || route.isEditingSegment(segment);
      } else {
        currentPart = null;
      }
    });
    visibleParts.sort(
      (part1, part2) =>
        (part1.editing ? 0 : part1.minDistanceToCenter) -
        (part2.editing ? 0 : part2.minDistanceToCenter),
    );
    ...
    camera.maxHeight = 0;
    for (
      let segmentIndex = mainVisiblePart.minIndex;
      segmentIndex <= mainVisiblePart.maxIndex;
      segmentIndex++
    ) {
      camera.minHeight = Math.min(
        camera.minHeight,
        route.segments[segmentIndex].z,
      );
      camera.maxHeight = Math.max(
        camera.maxHeight,
        route.segments[segmentIndex].z,
      );
    }
