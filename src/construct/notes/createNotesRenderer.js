...
import { LIMIT_TYPES, LIMIT_TYPES_TO_HIGHLIGHTS, LIMIT_TYPES_TO_LABEL, LIMIT_TYPE_MIN_RADIUS } from '../route/limitTypes.js';
  ...
  const formatLimitValue = (limitType) => {
    switch(limitType) {
      case LIMIT_TYPE_MIN_RADIUS:
        return `${level.limits[limitType]}m < ${route.limits[limitType].value < 1000 ? Math.round(route.limits[limitType].value) + 'm' : '∞'}`;
      default:
        return '?'  
    }
  };
