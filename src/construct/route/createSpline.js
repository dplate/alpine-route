const makeContinuousTangent = (previous, current, next) => {
  ...
  const factorToPrevious = Math.sqrt(
    distanceToPrevious.x ** 2 +
      distanceToPrevious.y ** 2 +
      distanceToPrevious.z ** 2,
  );
  const factorToNext = Math.sqrt(
    distanceToNext.x ** 2 + distanceToNext.y ** 2 + distanceToNext.z ** 2,
  );
  const factor = {
    x:
      (factorToNext * factorToNext * distanceToPrevious.x -
        factorToPrevious * factorToPrevious * distanceToNext.x) /
      2.75,
    y:
      (factorToNext * factorToNext * distanceToPrevious.y -
        factorToPrevious * factorToPrevious * distanceToNext.y) /
      2.75,
    z:
      (factorToNext * factorToNext * distanceToPrevious.z -
        factorToPrevious * factorToPrevious * distanceToNext.z) /
      3.0,
  };
...
const measureSegment = (
  ...
  const firstSubSegmentLength = Math.sqrt(
    (pointStart.x - pointCenter.x) ** 2 +
      (pointStart.y - pointCenter.y) ** 2 +
      (pointStart.z - pointCenter.z) ** 2,
  );
  const secondSubSegmentLength = Math.sqrt(
    (pointEnd.x - pointCenter.x) ** 2 +
      (pointEnd.y - pointCenter.y) ** 2 +
      (pointEnd.z - pointCenter.z) ** 2,
  );
  const detailLength = firstSubSegmentLength + secondSubSegmentLength;
  if (length === undefined || Math.abs(detailLength - length) > 0.1) {
    const firstSubSegmentMeasurement = measureSegment(
      evaluateSpline,
      tStart,
      pointStart,
      tCenter,
      pointCenter,
      firstSubSegmentLength,
    );
    const secondSubSegmentMeasurement = measureSegment(
      evaluateSpline,
      tCenter,
      pointCenter,
      tEnd,
      pointEnd,
      secondSubSegmentLength,
    );
    return {
      length:
        firstSubSegmentMeasurement.length + secondSubSegmentMeasurement.length,
      flatLength:
        firstSubSegmentMeasurement.flatLength +
        secondSubSegmentMeasurement.flatLength,
    };
  }
  return {
    length: detailLength,
...
export default (points) => {
  ...
  const measurement = segmentMeasurements.reduce(
  ...
  return {
    ...
    getLengthAtPointIndex: (pointIndex) => {
      return segmentMeasurements
        ...
        .reduce((sum, measurement) => sum + measurement.length, 0);
    },
    getFlatLengthAtPointIndex: (pointIndex) => {
      return segmentMeasurements
        .slice(0, pointIndex + 1)
        .reduce((sum, measurement) => sum + measurement.flatLength, 0);
    },
    getAtMeter: (meter) => {
      ...
      while (
        meter >
        totalSegmentLength + segmentMeasurements[segmentIndex + 1].length
      ) {
        segmentIndex++;
      ...
      return {
        ...
        flatMeter:
          totalSegmentFlatLength +
          fraction * segmentMeasurements[segmentIndex + 1].flatLength,
      };
    },
  };
};
