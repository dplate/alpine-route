...
export default (context, level, route, renderTarget) => {
  ...
  route.segments.forEach((segment, index) => {
    if (index >= 2 && index < route.segments.length - 2 && level.limits[LIMIT_TYPE_MIN_RADIUS] > segment.radius) {
      const center = calculateCenter(route.segments[index - 2], segment, route.segments[index + 2])
