...
const splitSection = (section) => {
  ...
  section.corners.forEach((corner, index) => {
    if (index > 0) {
      ...
      corner.startDistance = previousCorner.startDistance + Math.sqrt(
...
const drawSubSection = (context, section) => {
  ...
  const gradient = context.createLinearGradient(firstCorner.pixels.x, firstCorner.pixels.y, lastCorner.pixels.x, lastCorner.pixels.y); 
  ...
  if (section.type === ROUTE_TYPE_TUNNEL) {
    const lineDash = [5, 15];
    context.lineDashOffset = section.corners[0].startDistance % (lineDash[0] + lineDash[1]);
