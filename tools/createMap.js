...
const collectMapData = async (segmentUrls) => {
  ...
  const [, maxSegmentYString] = segmentUrls[segmentUrls.length - 1].match(
    /swissalti3d_\d+_\d+-(\d+)/,
  );
  const minSegmentX = parseInt(minSegmentXString);
  const maxSegmentY = parseInt(maxSegmentYString);
  const maxSegmentX =
    minSegmentX + Math.floor(textureWidth / (1000 / resolutionInMeter));
  const minSegmentY =
    maxSegmentY - Math.floor(textureHeight / (1000 / resolutionInMeter));
    ...
    for (let segmentY = minSegmentY; segmentY <= maxSegmentY; segmentY++) {
      ...
      rows.forEach((row) => {
        ...
        if (heightString) {
          ...
          const y =
            textureHeight -
            (absoluteY - minSegmentY * 1000 - 1) / resolutionInMeter -
            1;
...
const createTexture = async (mapData) => {
  ...
  for (let x = 0; x < textureWidth; x++) {
    for (let y = 0; y < textureHeight; y++) {
      ...
      const heightColor = Math.floor(0xffff & Math.round(point.height / 10));

      const index = (textureWidth * y + x) * 3;

      bitmap[index] = (heightColor >> 8) & 0xff;
      bitmap[index + 1] = heightColor & 0xff;
    }
  }
  texture.data = buffer;
...
const segmentUrls = fs
  .readFileSync(`${mapDirectory}/segmentUrls.csv`, "utf8")
  .toString()
  .split("\n");
